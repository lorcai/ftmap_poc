...
2. **Sync** appends ids to the shared **staging** textarea (deduped, caps via `TMAP_MSA.maxStaging`). **Highlight on plot** **replaces** the Lore selection: `clearTmapPlotSelection` then `addSelected` per staged id. Clear must call `removeSelected` with the same index field Faerun uses (`item.index` / `item.item.e.index`), not only `item.item.index`, or removals no-op and the old bug looked like “highlight only adds.”
